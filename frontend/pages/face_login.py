...
from components.ui_components import logo_mark, status_pill
from utils.mock_api import verify_face


def render() -> None:
    if "face_state" not in st.session_state:
        st.session_state.face_state = "idle"  # idle | processing | success | failure

    # Top back button row
    if st.button("← Back to Login", key="face_back_top"):
    ...
    with main_col1:
        ...
        st.markdown(
            ...
            <div class="privacy-note">🔒 We never store your face image.</div>
            """,
            unsafe_allow_html=True,
        )
    ...
    with main_col2:
        with st.container(key="face_camera_card"):
            if state in ("idle", "processing"):
                # Real Streamlit camera input widget
                ...
                if st.button("Verify Face", type="primary", key="verify_face_btn", use_container_width=True):
                    st.session_state.face_state = "processing"
                ...
                if st.button("Continue to Chat", type="primary", key="face_continue_chat", use_container_width=True):
                    user = st.session_state.get("face_verified_user", {"name": "Alex Morgan"})
                    st.session_state.user = {"name": user["name"], "email": "alex.morgan@example.com"}
