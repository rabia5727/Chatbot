...
class _FakeQuery:
    """Minimal stand-in for a supabase-py table query, covering only the
    insert/select/delete/eq/order/limit chain db/documents.py and
    rag/vector_store.py actually use."""
    ...
    def _matches(self):
        return [row for row in self._rows if all(row.get(c) == v for c, v in self._filters)]
...
class FakeSupabase:
    """Isolated in-memory Supabase double, scoped to one test -- never
    touches the real project, so tests can use arbitrary fake ids freely."""
...
def rag_db():
    fake = FakeSupabase()
    with patch("db.documents.supabase", fake), patch("rag.vector_store.supabase", fake):
        yield fake
